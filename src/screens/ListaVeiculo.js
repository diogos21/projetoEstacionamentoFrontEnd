...
function ListaVeiculo({ data, navigation, theme }) {
  const { colors } = theme
  ...
  async function excluirVeiculo(data) {
    let url = `${BACKEND}/veiculos/${data._id}`
    await fetch(url, {
      method: 'DELETE',
      headers: {
      ...
      .catch(function (error) {
        console.error('Houve um problema ao excluir o veiculo: ' + error.message);
      })
  }

  const alteraVeiculo = async (data) => {
    navigation.navigate('AlteraVeiculo',{ veiculo: data })
  }
...
export default withTheme(ListaVeiculo)